...
export class ProductDetailsComponent implements OnInit {
  ...
  changeSize(size) {
    ...
    d3.select(".mygrid").style("width", val);
  }
  ...
  draw() {
    ...
    let selsvg = d3.select(".child-svg").on("mouseover", function(d) {
    ...
    for (let entry of this.product.cars) {
      ...
      selsvg
        ...
        .on("mouseover", function(d) {
          console.log(
            "hello i am the brand :" + entry.name + ", value: " + entry.serial
          );
        }).on("click", function(d) {
          alert(
            "clicked: " + entry.name + ", value: " + entry.serial
          );
        });
